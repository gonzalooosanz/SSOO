...
{
    int numINodes;
	NodeStruct* structuraNod = malloc(sizeof (NodeStruct));

	for (numINodes = 0; numINodes < MAX_NODES; numINodes++) { // Recorremos la estructura
		readNode(myFileSystem, numINodes, structuraNod);
		if (structuraNod->freeNode) { // si el nodo está libre
		    myFileSystem->nodes[numINodes] = NULL;
		} else {// si el nodo no está libre
		    myFileSystem->numFreeNodes--; // decrementamos el número de nodos libres
		    myFileSystem->nodes[numINodes] = malloc(sizeof (NodeStruct));
		    copyNode(myFileSystem->nodes[numINodes], structuraNod);
		}
	}
