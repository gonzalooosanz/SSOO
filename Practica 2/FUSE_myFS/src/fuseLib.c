...
static int my_unlink(const char *path)
{
    ...
    for (int contador = 0; contador < tamBloque; ++contador) {
        
		puntero = myFileSystem.nodes[nodoI]->blocks[contador];
        myFileSystem.bitMap[puntero] = 0;
    }

    
	/*
	El nodo al que apunta la variable nodoI, tiene que tener nodos libres,
	además el número de bloques tiene que ser 0, por lo que el tamaño del fichero
	también tiene que ser 0
	*/
    ...
    myFileSystem.numFreeNodes++; // Incrementamos el número de nodos libres, en nuestro sistema de ficheros

    myFileSystem.superBlock.numOfFreeBlocks = myQuota(&myFileSystem); 


    myFileSystem.directory.files[idxNodoI].freeFile = true; // tiene nodos libres
    myFileSystem.directory.numFiles--; // El número de archivos del directorio tiene que decrementar en 1

    updateSuperBlock(&myFileSystem); // actualización del super Bloque
    updateDirectory(&myFileSystem); // actualización del directorio
    updateBitmap(&myFileSystem);   // actualización del mapa de bits
    updateNode(&myFileSystem, nodoI, myFileSystem.nodes[nodoI]); // actulaizamos los nodos i
    sync();
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    while(leer) {
        int contador;
        int bloqueActual = node->blocks[offset / BLOCK_SIZE_BYTES];
        int bloqueQuitado = offset % BLOCK_SIZE_BYTES;

        if(readBlock(&myFileSystem, bloqueActual, &buffer) == -1) { // Si se produce un error a la hora de leer un bloque
            fprintf(stderr,"Error reading blocks in my_read\n");
            return -EIO;
        }

        for(contador = bloqueQuitado; (contador < BLOCK_SIZE_BYTES) && (numLeidos < size); contador++) {
            buf[numLeidos++] = buffer[i];
        }

        leer = leer - (contador - bloqueQuitado); // actualización
        offset = offset + (contador - bloqueQuitado); // actualización
    }
...
struct fuse_operations myFS_operations = {
    ...
    .readdir	= my_readdir,					// Lee las entradas del directorio
    .truncate	= my_truncate,					// Modifica el tam de un archivo
    .open		= my_open,						// Abre un archivo
    .write		= my_write,						// Escribe los datos en un archivo ya abierto
    .release	= my_release,					// Cierra un archivo ya abierto
    .mknod		= my_mknod,						// Crea un nuevo archivo
    .unlink     = my_unlink,                    // Elimina un fichero
    .read       = my_read,                      // Lee información de un fichero abierto
};
